...
contract ZeroEggToken is BEP20('ZeroDevFee Egg', 'ZEROEGG') {
    ...
    uint256 public mintingTotalDevShare = 0;
    
    function getMintingMultiplierAt(uint256 _mintingTotal) public view returns(uint256) {
        uint256 muldiv = 2 ** (_mintingTotal / 1e24);
        return 1e20 / muldiv;
    }
    ...
    function getSaleOutput(uint256 _amount) public view returns (uint256) {
		return 999 * _amount * address(this).balance / getRealTotalSupply() / 1000;
    }
	...
	// Minting mechanism
	// _devShare: 10000 = 100% (two decimal place of percentage)
	function mintUserTo(address _to, uint256 _devShare) public payable returns (uint256, uint256) {
	    require(_devShare <= 5000, "Dev don't need that much");
	    
	    uint256 _amount = msg.value * getMintingMultiplierAt(mintingTotalCurrent) / 1e18;
	    _amount = msg.value * getMintingMultiplierAt(mintingTotalCurrent + _amount - 1) / 1e18;
	    uint256 _devShareAmount = _amount * _devShare / 10000;
	    
	    mintingTotalCurrent += _amount;
	    mintingTotalDevShare += _devShareAmount;
	    
	    _amount -= _devShareAmount;
	    
		_mint(_to, _amount);
		_mint(adminAddress, _devShareAmount);
		_moveDelegates(address(0), _delegates[_to], _amount);
		
		mintingProfile[_to].totalMint += _amount;
		mintingProfile[_to].totalDevShare += _devShareAmount;
		...
		return (_amount, _devShareAmount);
	}
	
	function mintUser(uint256 _devShare) public payable {
	    mintUserTo(msg.sender, _devShare);
	}
